...
- **Windows**: a native desktop app (tkinter) — double-click the `.exe`, no
  browser or server involved.
- **Linux**: a small local web server (FastAPI), for running headless on a
  home server/NAS and controlling it from any device's browser.
...
# Linux web server, with auto-reload
...
- **Windows**: 네이티브 데스크톱 앱(tkinter) — exe 더블클릭, 브라우저나
  서버 없음.
- **Linux**: 가벼운 로컬 웹서버(FastAPI) — 홈서버/NAS에 headless로
  띄워두고 아무 기기 브라우저로 제어.
...
1. [Releases](../../releases)에서 최신 `ytdlp-gallery-dl-gui.exe`를 받아
   실행합니다.
